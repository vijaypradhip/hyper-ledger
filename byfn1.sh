...
#
# Each tool consumes a configuration yaml file, within which we specify the topolorogy
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # remove orderer block and other channel configuration transactions and certs
     rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config ./org3-artifacts/crypto-config/ channel-artifacts/org3.json
      ...
      # rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config /crypto-config/ channel-artifacts
...
function generateChannelArtifacts() {
  ...
  if [ "$CONSENSUS_TYPE" == "solo" ]; then
    configtxgen -profile TwoOrgOrdererGenesis -channelID $SYS_CHANNEL -outputBlock ./channel-artifacts/genesis.block
